...
The specs are also published as a browsable wiki: **[wiki/index.md](wiki/index.md)**

It is generated by two Claude Code skills included in this repo:

- **[spec-wiki-builder](docs/spec-wiki-skills.md#spec-wiki-builder)** — reads every folder under `specs/` and produces a wiki page per feature plus a categorised index. Runs incrementally; only update features without wiki pages.

- **[spec-wiki-optimizer](docs/spec-wiki-skills.md#spec-wiki-optimizer)** — finds specs that are fixes or updates of an earlier feature, merges them into a single consolidated page, and removes the now-redundant individual pages from the index.

Run `/spec-wiki-builder` then `/spec-wiki-optimizer` in Claude Code to regenerate the wiki after adding or changing specs. Rebuild operations are supported for both skills.
