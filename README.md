...
- **[spec-wiki-builder](docs/spec-wiki-skills.md#spec-wiki-builder)** — reads every folder under `specs/` and produces a wiki page per feature plus a categorised index. Runs incrementally; only rebuilds pages whose source files have changed.
...
Run `/spec-wiki-builder` then `/spec-wiki-optimizer` in Claude Code to regenerate the wiki after adding or changing specs.
